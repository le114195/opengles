...
int EDGE_DETECTION::Init ( ESContext *esContext ) {
...
void EDGE_DETECTION::Draw ( ESContext *esContext ) {
    ...
    GLint mLoc = glGetUniformLocation(userData->programObject, "convolutionMatrix");
    ...
    glUniformMatrix3fv(mLoc, 1, GL_FALSE, &mValue[0][0]);
...
void EDGE_DETECTION::Update ( ESContext *esContext, float deltaTime ) {
...
// Cleanup
//
void EDGE_DETECTION::ShutDown ( ESContext *esContext )
{
    UserData *userData = (UserData *)esContext->userData;
...
int EDGE_DETECTION::esMain(ESContext *esContext) {
    if ( !Init ( esContext ) )
    {
        return GL_FALSE;
    }
